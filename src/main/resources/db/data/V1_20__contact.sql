...
select (select max(ADDRESS_ID)+1 from address), 'CRS', SYSDATE, (SELECT USER_ID FROM USER_ WHERE DISTINGUISHED_NAME = 'DELIUS_SYSTEM_USER'), SYSDATE, (SELECT USER_ID FROM USER_ WHERE DISTINGUISHED_NAME = 'DELIUS_SYSTEM_USER'), 0, 0
FROM DUAL
WHERE NOT EXISTS
...
select (select max(ORGANISATION_ID)+1 from ORGANISATION), 'CRS', 'Commissioned Rehabilitive Services Providers', TO_DATE('01/01/2021', 'DD/MM/YYYY'), 1,
...
select (select max(PROBATION_AREA_ID)+1 from PROBATION_AREA), 'CRS', 'Commissioned Rehabilitive Services Provider', 'N', 1, (select ORGANISATION_ID from ORGANISATION where CODE = 'CRS'), (select address_id from address where BUILDING_NAME='CRS'), TO_DATE('01/01/2021', 'DD/MM/YYYY'),
...
select (select max(BOROUGH_ID)+1 from borough), ''||pa.code || 'UAT', 'Unallocated Cluster', 'Y',1, (select user_id from user_ where distinguished_name = 'DELIUS_SYSTEM_USER'), sysdate,
...
select (select max(DISTRICT_ID)+1 from district), ''||pa.code || 'UAT', 'Unallocated LDU', 'Y', (select borough_id from borough where code = ''||pa.code || 'UAT'), 1,
...
select (select max(LOCAL_DELIVERY_UNIT_ID)+1 from local_delivery_unit), ''||pa.code || 'UAT', 'Unallocated Team Type', 'Y', 1, (select user_id from user_ where distinguished_name = 'DELIUS_SYSTEM_USER'), sysdate,
       (select user_id from user_ where distinguished_name = 'DELIUS_SYSTEM_USER'), sysdate, probation_area_id from probation_area pa where pa.code = 'CRS'
...
select (select max(team_id)+1 from team), ''||pa.code || 'UAT', 'Unallocated', (select district_id from district where code = ''||pa.code||'UAT'), (select local_delivery_unit_id from local_delivery_unit where code = ''||pa.code||'UAT'), 'Y', 1,
       sysdate, (select user_id from user_ where distinguished_name = 'DELIUS_SYSTEM_USER'), sysdate,
...
select (select max(staff_id)+1 from staff), trunc(sysdate), 'Unallocated', 'Unallocated', 1, ''||pa.code||'UATU', (select user_id from user_ where distinguished_name = 'DELIUS_SYSTEM_USER'),
...
select (select staff_id from staff where officer_code = ''||pa.code||'UATU'), (select team_id from team where code = ''||pa.code||'UAT'), 1,
       (select user_id from user_ where distinguished_name = 'DELIUS_SYSTEM_USER'), sysdate, (select user_id from user_ where distinguished_name = 'DELIUS_SYSTEM_USER'),
       sysdate
from probation_area pa where pa.code = 'CRS'
;